...
interface FabricComplaintTableProps {
  user: {
    role: Role;
    full_name: string;
    id: string;
...
export default function FabricComplaintTable({
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/api/fabric-complaints/${id}?deleted_by=${encodeURIComponent(user.full_name)}`, {
        method: "DELETE",
      });
      const json = await res.json();
      if (json.success) {
