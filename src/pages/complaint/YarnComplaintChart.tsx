...
export default function YarnComplaintChart({
  ...
  const chartData = useMemo(() => {
    ...
    const aggregate = (items: YarnComplaint[]) => {
      ...
      <div className={isHorizontal ? "h-[800px]" : ((type === "nature" || type === "customer") ? "h-96" : "h-64")}>
        <ResponsiveContainer width="100%" height="100%" minWidth={0} minHeight={0}>
          ...
          >
            ...
            {isHorizontal ? (
              <>
                <XAxis type="number" fontSize={11} tick={{ fill: '#4b5563' }} axisLine={{ stroke: '#e5e7eb' }} tickFormatter={(val) => calcMode === "per100mt" ? formatValue(val, calcMode) : val} />
              ...
              <>
                <XAxis 
                  ...
                  interval={0}
                />
                <YAxis fontSize={11} tick={{ fill: '#4b5563' }} axisLine={{ stroke: '#e5e7eb' }} tickFormatter={(val) => calcMode === "per100mt" ? formatValue(val, calcMode) : val} label={{ value: yAxisLabel, angle: -90, position: 'insideLeft', offset: 0, fontSize: 10, fill: '#9ca3af' }} />
              </>
            )}
            <Tooltip 
              cursor={{ fill: '#f9fafb' }} 
              ...
              formatter={(value: any) => {
                ...
                if (calcMode === "per100mt") {
                  return [formatValue(value, calcMode), "Complaints per 100 MT"];
                }
            ...
            <Bar dataKey={activeDataKey} name={yAxisLabel} radius={isHorizontal ? [0, 4, 4, 0] : [4, 4, 0, 0]} className="cursor-pointer">
              ...
              <LabelList 
                ...
                formatter={(value: any) => {
                  ...
                  if (calcMode === "per100mt") {
                    return formatValue(value, calcMode);
                  }
                  return value;
                }}
              />
              ...
              <Pie
                ...
                label={({ name, value, percent, index }) => {
                  ...
                  if (calcMode === "per100mt") {
                    ...
                    const key = String(item[dataKey]);
                    const billedQty = statsForType ? (statsForType[key] || 0) : totalBilled;
                    const pValue = billedQty > 0 ? (Number(item.count) / billedQty) * 100 : 0;
                    return `${name}: ${formatValue(pValue, calcMode)}`;
                  }
                  return `${name} (${value})`;
                }}
                onClick={(state) => {
              ...
              <Tooltip 
                ...
                formatter={(value: any, name: any, props: any) => {
                  ...
                  if (calcMode === "per100mt") {
                    ...
                    const billedQty = statsForType ? (statsForType[key] || 0) : totalBilled;
                    const pValue = billedQty > 0 ? (Number(item.count) / billedQty) * 100 : 0;
                    return [formatValue(pValue, calcMode), "Complaints per 100 MT"];
                  }
                  return [value, "Total Complaints"];
                }}
              />
  ...
  return (
    <div className="p-2 space-y-4">
      {/* Toolbar consistent with YarnComplaintTable.tsx */}
      <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm flex flex-col gap-4">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="relative" ref={filterDropdownRef}>
            ...
            {activeFilterCol && (
              <div ref={filterValuesRef} className="absolute top-10 left-60 z-50 bg-white border border-gray-200 rounded-lg shadow-2xl p-2 w-64 animate-in fade-in slide-in-from-left-2">
                ...
                <div className="max-h-60 overflow-y-auto px-1">
                  {loadingOptions ? (
                  ...
                  ) : filterOptions.filter(opt => opt.toLowerCase().includes(filterSearch.toLowerCase())).length > 0 ? (
                    filterOptions.filter(opt => opt.toLowerCase().includes(filterSearch.toLowerCase())).map((opt) => (
                      <div
                        key={opt}
                        className={`px-3 py-2 text-xs cursor-pointer rounded hover:bg-red-50 hover:text-red-700 transition-colors flex items-center gap-3 ${selectedFilters[activeFilterCol]?.includes(opt) ? 'bg-red-50 text-red-700 font-medium' : 'text-gray-600'}`}
                    ...
                    <div className="px-3 py-4 text-center text-xs text-gray-400 italic">No results found</div>
                  )}
                </div>
                {selectedFilters[activeFilterCol] && (
                  <div className="mt-2 pt-2 border-t border-gray-50">
                    <button
                      ...
                      className="w-full text-center py-1.5 text-xs text-red-600 hover:bg-red-50 rounded font-bold"
                    >
                      Clear {activeFilterCol.replaceAll("_", " ")}
                    </button>
                  </div>
                )}
              </div>
