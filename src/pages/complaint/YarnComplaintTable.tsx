...
const REQUIRED_YARN_FIELDS = ["query_received_date", "status", "customer_name", "customer_type"];
...
export default function YarnComplaintTable({
  ...
  const handleDownloadTemplate = () => {
    ...
    link.setAttribute("download", "yarn_complaint_template.csv");
  ...
  const handleDownloadExcel = () => {
    ...
    XLSX.writeFile(workbook, "yarn_complaints.xlsx");
  };

  const handleDownloadPDF = () => {
    const headerText = prompt("Enter PDF Header:", "Yarn Complaints Report");
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/api/yarn-complaints?${params.toString()}`);
      const json = await res.json();
  ...
  const fetchUniqueValues = async (col: string) => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/api/unique-values/${TABLE_NAME}/${col}`);
      
      // Check if response is JSON
      const contentType = res.headers.get("content-type");
      if (!contentType || !contentType.includes("application/json")) {
        throw new Error("Backend returned non-JSON response. Check if server is running.");
      }
      ...
      alert(`Error: ${err instanceof Error ? err.message : "Unknown error"}`);
      setActiveFilterCol(null); // Close modal on error
    } finally {
  ...
  const allColumns = useMemo(() => {
    return ALL_YARN_COLUMNS;
  }, []);

  const totalWidth = useMemo(() => visibleColumns.reduce((sum, col) => sum + (columnWidths[col] || 160), 0), [visibleColumns, columnWidths]);
  const filteredOptions = useMemo(() => filterOptions.filter(opt => opt.toLowerCase().includes(filterSearch.toLowerCase())), [filterOptions, filterSearch]);
  ...
  return (
    <div className="flex flex-col h-full bg-white rounded-xl shadow-sm border border-gray-200">
      ...
      {/* ===== Footer / Stats ===== */}
      <div className="px-4 py-2 border-t border-gray-100 bg-white flex justify-between items-center text-xs text-gray-500">
        <div>Showing {data.length} records</div>
        <div className="flex gap-4">
          <span>{visibleColumns.length} columns visible</span>
          {Object.keys(selectedFilters).length > 0 && (
            <span className="text-red-600 font-medium">{Object.keys(selectedFilters).length} filters active</span>
          )}
        </div>
      </div>
      ...
      {activeFilterCol && (
        <div className="fixed inset-0 bg-gray-900/50 backdrop-blur-sm flex items-center justify-center z-50 animate-in fade-in duration-200">
          <div className="bg-white rounded-xl shadow-2xl w-[400px] border border-gray-100 flex flex-col max-h-[80vh]">
            <div className="p-4 border-b border-gray-100 flex items-center justify-between">
               <div>
                 <h3 className="text-lg font-bold text-gray-900 capitalize">
                   Filter {activeFilterCol.replaceAll("_", " ")}
                 </h3>
                 <p className="text-xs text-gray-500">Select unique values to filter</p>
               </div>
               <button onClick={() => setActiveFilterCol(null)} className="p-1 hover:bg-gray-100 rounded-full transition-colors">
            ...
            <div className="p-4 bg-gray-50 border-b border-gray-100">
               <div className="relative">
                 ...
                 <input
                   ...
                   className="w-full bg-white border border-gray-300 rounded-lg pl-10 pr-4 py-2 text-sm focus:ring-2 focus:ring-red-500 focus:border-red-500 outline-none transition-all"
                 />
               </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-2">
              {loadingOptions ? (
                <div className="flex flex-col items-center justify-center py-12 gap-3">
                  ...
                  <span className="text-sm text-gray-500 font-medium">Fetching unique values...</span>
                </div>
              ) : filteredOptions.length > 0 ? (
                <div className="space-y-0.5">
                   <button
                    onClick={() => {
                      const current = selectedFilters[activeFilterCol] || [];
                      const allVisibleSelected = filteredOptions.every(opt => current.includes(opt));
                      
                      setSelectedFilters(prev => ({
                        ...prev,
                        [activeFilterCol]: allVisibleSelected 
                          ? current.filter(v => !filteredOptions.includes(v))
                          : Array.from(new Set([...current, ...filteredOptions]))
                      }));
                    }}
                    className="w-full text-left px-3 py-2 text-xs font-bold text-red-600 hover:bg-red-50 rounded transition-colors"
                  >
                    {filteredOptions.every(opt => (selectedFilters[activeFilterCol] || []).includes(opt)) ? "Deselect All Visible" : "Select All Visible"}
                  </button>
                  {filteredOptions.map((opt) => (
                    <label key={opt} className="flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-gray-50 cursor-pointer transition-colors group">
                      <input
                        type="checkbox"
                        className="w-4 h-4 rounded border-gray-300 text-red-600 focus:ring-red-500"
                      ...
                      />
                      <span className="text-sm text-gray-700 group-hover:text-gray-900 font-medium">{opt}</span>
                    </label>
                  ))}
                </div>
              ) : (
                <div className="text-center py-12 text-gray-400 italic text-sm">No values found matching your search.</div>
              )}
            </div>

            <div className="p-4 border-t border-gray-100 flex justify-between items-center bg-gray-50/50">
               <span className="text-xs text-gray-500">{(selectedFilters[activeFilterCol] || []).length} selected</span>
               <div className="flex gap-2">
                 ...
                 <button
                   ...
                   className="px-4 py-1.5 text-xs font-medium bg-red-600 text-white rounded-md hover:bg-red-700 transition-colors shadow-sm"
                 >
                   Apply Filters
                 </button>
               </div>
      ...
      {showEditModal && editingRow && (
        <div className="fixed inset-0 bg-gray-900/50 backdrop-blur-sm flex items-center justify-center z-50 animate-in fade-in duration-200">
          <div className="bg-white rounded-xl shadow-2xl w-[90%] max-w-6xl border border-gray-100 flex flex-col max-h-[90vh]">
            <div className="p-4 border-b border-gray-100 flex items-center justify-between">
              <div className="flex items-center gap-4">
                <div>
                  <h3 className="text-lg font-bold text-gray-900">
                    {editingRow.id ? "Edit Complaint" : "Add New Complaint"}
                  </h3>
                  <p className="text-xs text-gray-500">
                    {editingRow.id ? "Update the details of the yarn complaint" : "Fill in the details to record a new yarn complaint"}
                  </p>
                </div>
            ...
            <form 
              onSubmit={async (e) => {
                ...
                // Validation
                const missingFields = REQUIRED_YARN_FIELDS.filter(f => !editingRow[f]);
                  ...
                  return (
                    <div key={col} className="flex flex-col gap-1.5">
                      <label className="text-xs font-bold text-gray-500 uppercase tracking-wider">
                        {col.replaceAll("_", " ")}
                        {REQUIRED_YARN_FIELDS.includes(col) && <span className="text-red-500 ml-1">*</span>}
                      </label>
                      {isDropdown ? (
                        <SearchableDropdown
                                ...
                                try {
                                  ...
                                  if (json.success) {
                                    ...
                                    setEditingRow({
                                      ...
                                      yarn_lot_no: d.lot_no || editingRow.yarn_lot_no,
